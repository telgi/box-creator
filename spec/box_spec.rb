...
describe Box do
  ...
  describe '#create_box' do
    ...
    it 'does not allow dimensions less than 2x2' do
      expect { box.create(1, 1) }.to raise_error("You must select dimensions that are greater than or equal to 2x2")
    end

    it 'does only allows integers to be passed through as arguments for width and height' do
      expect { box.create(3.7, 4.8) }.to raise_error("You must select dimensions with whole numbers")
    end
  end
end
